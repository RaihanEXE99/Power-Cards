...
class StartingGame(LoginRequiredMixin,View): 
    def initiateGame(self,lobby):
        ...
        players=[user.username for user in lobby.users_m.all()]
        total_players = len(players)
        cards = []
        for _ in range(total_players):
            cards.append(randomCardList(10))
        gameJson = {
            "status":"in-game",
            ...
            "players":players,
            "cards":cards,
            # "turn":random.randint(0,total_players-1),
            "turn":0,
            "points":[0 for _ in range(total_players)],
...
class InGame(LoginRequiredMixin, View):
    def get(self, request, room_name, *args, **kwargs):
        exist = get_object_or_404(Lobby, room_name=room_name,status="started",gameJson__status="in-game")
        userlist = [user.username for user in exist.users_m.all()]
